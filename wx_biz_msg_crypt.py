...
class SHA1:
    ...
    def getSHA1(self, token, timestamp, nonce, encrypt):
        ...
        """
        try:
            sl = [token, timestamp, nonce, encrypt]
            sl.sort()
            sha = hashlib.sha1()
            sha.update("".join(sl).encode("utf8"))
...
class Prpcrypt(object):
    ...
    def encrypt(self, text, receive_id):
        text = MYAESCipher.get_iv() + struct.pack("I", socket.htonl(len(text.encode()))) \
               + text.encode() + receive_id.encode()
        try:
            cf = MYAESCipher(self.key, self.key[:16], 32)
            return WXBizMsgCrypt_OK, cf.encrypt(text)
        except Exception as e:
            print('Prpcrypt', e)
    ...
    def decrypt(self, text, receive_id):
        ...
        try:
            ...
            from_receive_id = content[xml_len+4:].decode()
        except Exception as e:
            print('Prpcrypt decry', e)
